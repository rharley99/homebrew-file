...
""")
    ...
    if 'HOMEBREW_BREW_FILE_TOP_PACKAGES' in env:
        formulae = ['gettext', 'git', 'node', 'pcre2']
    elif 'HOMEBREW_BREW_FILE_ON_REQUEST' in env:
        formulae = ['gettext', 'git', 'node']
    elif 'HOMEBREW_BREW_FILE_LEAVES' in env:
        formulae = ['git', 'node']
    formulae_list = '\n'.join(f'brew {x}' for x in formulae)
    formulae_part = f"""
...
def test_codium(
    ...
    with Path(brewfile).open('w') as f:
        f.write("""
codium amazonwebservices.aws-toolkit-vscode
